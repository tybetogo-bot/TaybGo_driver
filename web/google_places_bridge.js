(function () {
  ...
  function addressFields(components) {
    ...
    (components || []).forEach(function (component) {
      ...
      if (types.includes('street_number')) fields.houseNumber = componentValue(component);
      ...
      if (!fields.city && types.includes('administrative_area_level_2')) {
        fields.city = componentValue(component);
      }
      if (types.includes('postal_code')) fields.postalCode = componentValue(component);
      if (types.includes('country')) {
        fields.country = component.shortText || component.short_name || componentValue(component);
      }
    });
    return fields;
  }
    ...
    try {
      ...
      return (response.suggestions || []).map(function (suggestion) {
        ...
        return {
          ...
          fullText: prediction.text ? prediction.text.text : '',
          primaryText: prediction.mainText ? prediction.mainText.text : '',
  ...
  window.taybgoReverseGeocodeCountry = async function (apiKey, latitude, longitude) {
    ...
    for (const result of response.results || []) {
      const fields = addressFields(result.address_components);
      if (fields.country) return fields.country;
    }
    return null;
  };
})();
